...
import { Alert, DetailRow, EmptyState, StatusBadge } from "@/components/ui";
...
import type { Destination, Reservation, Vehicle, WaitlistEntryWithRefs } from "@/lib/types";
...
export default async function ReservationsPage({
  ...
  const [{ data }, { data: waitlistRows }, { data: destinationRows }] = await Promise.all([
    supabase
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-soft">
          Everything you have requested, and what the office decided.
        </p>
    ...
    <article className="card overflow-hidden md:flex">
      ...
      <div className="flex-1 p-5">
        ...
        <dl className="mt-3">
          ...
          {r.fuel_out !== null ? (
            <DetailRow label="Bring it back at">{fuelLabel(r.fuel_out)}</DetailRow>
          ) : null}
          {r.late_fee_cents > 0 ? (
            <DetailRow label="Late fee">{formatMoney(r.late_fee_cents)}</DetailRow>
          ) : null}
          {r.fuel_fee_cents > 0 ? (
            <DetailRow label="Fuel fee">{formatMoney(r.fuel_fee_cents)}</DetailRow>
          ) : null}
